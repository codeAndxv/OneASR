...
class LoadEngineRequest(BaseModel):
    """加载引擎请求。"""
    engine: str = Field(..., description="Provider 名称，如 whisper1")
...
async def api_load_engine(req: LoadEngineRequest):
    """加载指定 Provider 和模型。"""
    logger.info(
        "[engines/load] 请求加载引擎: provider=%s, model=%s, device=%s, compute_type=%s",
        req.engine, req.model, req.device, req.compute_type,
    )
    t_start = time.time()
    try:
        loaded = load_engine(
            provider_name=req.engine,
        ...
        logger.info(
            ...
            loaded.provider_name, loaded.model_name, loaded.device, loaded.compute_type,
